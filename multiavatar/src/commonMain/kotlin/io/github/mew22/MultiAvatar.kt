...
@JvmInline
value class AvatarDataPart private constructor(val value: Int) {
    companion object : ValueObjectCreator<Int, AvatarDataPart, IllegalArgumentException>() {
        override val construction: (Int) -> AvatarDataPart = ::AvatarDataPart
        override fun isValid(input: Int): Boolean = input in 0..MAX
        const val MAX = 47
        fun getRandomPart(random: Random = Random(Clock.System.now().toEpochMilliseconds())) =
            AvatarDataPart((0..MAX).random(random))
    }

    fun increment() = AvatarDataPart((value + 1) % (MAX + 1))
    fun decrement() = AvatarDataPart((MAX + value - 1) % (MAX + 1))
}

data class AvatarData(
    val background: AvatarDataPart,
    val clothes: AvatarDataPart,
    val head: AvatarDataPart,
    val mouth: AvatarDataPart,
    val eyes: AvatarDataPart,
    val top: AvatarDataPart,
) {
    companion object {
        fun generateWithSha256(input: String): Result<AvatarData> = runCatchingSafe {
            val hash = MultiAvatar.sha256Hash(input)
            val backgroundHex =
                AvatarDataPart.getOrThrow((0.47 * hash.substring(0, 2).toInt()).roundToInt())
            val clothesHex =
                AvatarDataPart.getOrThrow((0.47 * hash.substring(2, 4).toInt()).roundToInt())
            val headHex =
                AvatarDataPart.getOrThrow((0.47 * hash.substring(4, 6).toInt()).roundToInt())
            val mouthHex =
                AvatarDataPart.getOrThrow((0.47 * hash.substring(6, 8).toInt()).roundToInt())
            val eyesHex =
                AvatarDataPart.getOrThrow((0.47 * hash.substring(8, 10).toInt()).roundToInt())
            val topHex =
                AvatarDataPart.getOrThrow((0.47 * hash.substring(10, 12).toInt()).roundToInt())
            AvatarData(backgroundHex, clothesHex, headHex, mouthHex, eyesHex, topHex)
        }

        fun generateFromFormattedString(input: String): Result<AvatarData> = runCatchingSafe {
            val parts = input.split("|")
            AvatarData(
                background = AvatarDataPart.getOrThrow(parts[0].toInt()),
                clothes = AvatarDataPart.getOrThrow(parts[1].toInt()),
                head = AvatarDataPart.getOrThrow(parts[2].toInt()),
                mouth = AvatarDataPart.getOrThrow(parts[3].toInt()),
                eyes = AvatarDataPart.getOrThrow(parts[4].toInt()),
                top = AvatarDataPart.getOrThrow(parts[5].toInt()),
            )
        }

        fun generateRandom(): AvatarData = AvatarData(
            background = AvatarDataPart.getRandomPart(),
            clothes = AvatarDataPart.getRandomPart(),
            head = AvatarDataPart.getRandomPart(),
            mouth = AvatarDataPart.getRandomPart(),
    ...
    override fun toString(): String =
        "${background.value}|${clothes.value}|${head.value}|" +
                "${mouth.value}|${eyes.value}|${top.value}"
}

object MultiAvatar {

    val themes: Map<String, Map<String, Map<String, List<String>>>> = mutableMapOf(
        // Robo
        "00" to mutableMapOf(
            "A" to mutableMapOf(
                "env" to listOf("#ff2f2b"),
                "clo" to listOf("#fff", "#000"),
                "head" to listOf("#fff"),
                "mouth" to listOf("#fff", "#000", "#000"),
                "eyes" to listOf("#000", "none", "#00FFFF"),
                "top" to listOf("#fff", "#fff")
            ),
            "B" to mutableMapOf(
                "env" to listOf("#ff1ec1"),
                "clo" to listOf("#000", "#fff"),
                "head" to listOf("#ffc1c1"),
                "mouth" to listOf("#fff", "#000", "#000"),
                "eyes" to listOf("#FF2D00", "#fff", "none"),
                "top" to listOf("#a21d00", "#fff")
            ),
            "C" to mutableMapOf(
                "env" to listOf("#0079b1"),
                "clo" to listOf("#0e00b1", "#d1fffe"),
                "head" to listOf("#f5aa77"),
                "mouth" to listOf("#fff", "#000", "#000"),
                "eyes" to listOf("#0c00de", "#fff", "none"),
                "top" to listOf("#acfffd", "#acfffd")
            )
        ),
        ...
        "01" to mutableMapOf(
            "A" to mutableMapOf(
                "env" to listOf("#a50000"),
                "clo" to listOf("#f06", "#8e0039"),
                "head" to listOf("#85492C"),
                "mouth" to listOf("#000"),
                "eyes" to listOf("#000", "#ff9809"),
                "top" to listOf("#ff9809", "#ff9809", "none", "none")
            ),
            "B" to mutableMapOf(
                "env" to listOf("#40E83B"),
                "clo" to listOf("#00650b", "#62ce5a"),
                "head" to listOf("#f7c1a6"),
                "mouth" to listOf("#6e1c1c"),
                "eyes" to listOf("#000", "#ff833b"),
                "top" to listOf("#67FFCC", "none", "none", "#ecff3b")
            ),
            "C" to mutableMapOf(
                "env" to listOf("#ff2c2c"),
                "clo" to listOf("#fff", "#000"),
                "head" to listOf("#ffce8b"),
                "mouth" to listOf("#000"),
                "eyes" to listOf("#000", "#0072ff"),
                "top" to listOf("#ff9809", "none", "#ffc809", "none")
            )
        ),
        ...
        "02" to mutableMapOf(
            "A" to mutableMapOf(
                "env" to listOf("#ff7520"),
                "clo" to listOf("#d12823"),
                "head" to listOf("#fee3c5"),
                "mouth" to listOf("#d12823"),
                "eyes" to listOf("#000", "none"),
                "top" to listOf("#000", "none", "none", "#FFCC00", "red")
            ),
            "B" to mutableMapOf(
                "env" to listOf("#ff9700"),
                "clo" to listOf("#000"),
                "head" to listOf("#d2ad6d"),
                "mouth" to listOf("#000"),
                "eyes" to listOf("#000", "#00ffdc"),
                "top" to listOf("#fdff00", "#fdff00", "none", "none", "none")
            ),
            "C" to mutableMapOf(
                "env" to listOf("#26a7ff"),
                "clo" to listOf("#d85cd7"),
                "head" to listOf("#542e02"),
                "mouth" to listOf("#f70014"),
                "eyes" to listOf("#000", "magenta"),
                // "eyes": ["#000", "#ff0000"],
                "top" to listOf("#FFCC00", "#FFCC00", "#FFCC00", "#ff0000", "yellow")
            )
        ),
        ...
        "03" to mutableMapOf(
            "A" to mutableMapOf(
                "env" to listOf("#6FC30E"),
                "clo" to listOf("#b4e1fa", "#5b5d6e", "#515262", "#a0d2f0", "#a0d2f0"),
                // "clo": ["#1f9ee4", "#262c5f", "#262c5f", "#a0d2f0", "#a0d2f0"],
                "head" to listOf("#fae3b9"),
                "mouth" to listOf("#fff", "#000"),
                "eyes" to listOf("#000"),
                "top" to listOf("#8eff45", "#8eff45", "none", "none")
            ),
            "B" to mutableMapOf(
                "env" to listOf("#00a58c"),
                "clo" to listOf("#000", "#005b00", "#005100", "#00a000", "#00a000"),
                "head" to listOf("#FAD2B9"),
                "mouth" to listOf("#fff", "#000"),
                "eyes" to listOf("#000"),
                "top" to listOf("#FFC600", "none", "#FFC600", "none")
            ),
            "C" to mutableMapOf(
                "env" to listOf("#ff501f"),
                "clo" to listOf("#000", "#ff0000", "#ff0000", "#7d7d7d", "#7d7d7d"),
                "head" to listOf("#fff3dc"),
                "mouth" to listOf("#d2001b", "none"),
                "eyes" to listOf("#000"),
                "top" to listOf("#D2001B", "none", "none", "#D2001B")
            )
        ),
        ...
        "04" to mutableMapOf(
            "A" to mutableMapOf(
                "env" to listOf("#fc0"),
                "clo" to listOf("#901e0e", "#ffbe1e", "#ffbe1e", "#c55f54"),
                "head" to listOf("#f8d9ad"),
                "mouth" to listOf("#000", "none", "#000", "none"),
                "eyes" to listOf("#000"),
                // "top": ["#48362D", "#AF892E", "#674C43", "#282828"]
                "top" to listOf("#583D00", "#AF892E", "#462D00", "#a0a0a0")
            ),
            "B" to mutableMapOf(
                "env" to listOf("#386465"),
                "clo" to listOf("#fff", "#333", "#333", "#333"),
                "head" to listOf("#FFD79D"),
                "mouth" to listOf("#000", "#000", "#000", "#000"),
                "eyes" to listOf("#000"),
                "top" to listOf("#27363C", "#5DCAD4", "#314652", "#333")
                // "top": ["#95E8CF", "#12353D", "#09A4AF", "#333"]
            ),
            "C" to mutableMapOf(
                "env" to listOf("#DFFF00"),
                "clo" to listOf("#304267", "#aab0b1", "#aab0b1", "#aab0b1"),
                "head" to listOf("#e6b876"),
                "mouth" to listOf("#50230a", "#50230a", "#50230a", "#50230a"),
                "eyes" to listOf("#000"),
                "top" to listOf("#333", "#afafaf", "#222", "#6d3a1d")
            )
        ),
        ...
        "05" to mutableMapOf(
            "A" to mutableMapOf(
                ...
                "head" to listOf("#f5d4a6"),
                "mouth" to listOf("#000", "#cf9f76"),
                "eyes" to listOf(
                    "#000",
                    "#000",
                    "#000",
                ...
                "top" to listOf("none", "#fdff00")
            ),
            "B" to mutableMapOf(
                "env" to listOf("#b3003e"),
                "clo" to listOf("#000", "#435363", "#435363", "#000", "none", "#e7ecf2", "#e7ecf2"),
                "head" to listOf("#f5d4a6"),
                "mouth" to listOf("#000", "#af9f94"),
                "eyes" to listOf(
                    "#9ff3ffdb",
                    "#000",
                    "#9ff3ffdb",
                ...
                "top" to listOf("#ff9a00", "#ff9a00")
            ),
            "C" to mutableMapOf(
                "env" to listOf("#884f00"),
                "clo" to listOf(
                    "#ff0000",
                    "#fff",
                    "#fff",
        ...
        "06" to mutableMapOf(
            "A" to mutableMapOf(
                "env" to listOf("#8acf00"),
                "clo" to listOf("#ee2829", "#ff0"),
                "head" to listOf("#ffce73"),
                "mouth" to listOf("#fff", "#000"),
                "eyes" to listOf("#000"),
                "top" to listOf("#000", "#000", "none", "#000", "#ff4e4e", "#000")
            ),
            "B" to mutableMapOf(
                "env" to listOf("#00d2a3"),
                // "env": ["#007359"],
                "clo" to listOf("#0D0046", "#ffce73"),
                "head" to listOf("#ffce73"),
                "mouth" to listOf("#000", "none"),
                "eyes" to listOf("#000"),
                "top" to listOf("#000", "#000", "#000", "none", "#ffb358", "#000", "none", "none")
                // "top": ["#00898c", "#fff"]
            ),
            "C" to mutableMapOf(
                // "env": ["#C13D5C"],
                "env" to listOf("#ff184e"),
                "clo" to listOf("#000", "none"),
                "head" to listOf("#ffce73"),
                "mouth" to listOf("#ff0000", "none"),
                "eyes" to listOf("#000"),
                "top" to listOf("none", "none", "none", "none", "none", "#ffc107", "none", "none")
            )
        ),
        ...
        "07" to mutableMapOf(
            "A" to mutableMapOf(
                "env" to listOf("#00deae"),
                // "clo": ["#cc8562", "#f2efea", "#cc8562"],
                "clo" to listOf("#ff0000"),
                "head" to listOf("#ffce94"),
                "mouth" to listOf("#f73b6c", "#000"),
                "eyes" to listOf("#e91e63", "#000", "#e91e63", "#000", "#000", "#000"),
                "top" to listOf("#dd104f", "#dd104f", "#f73b6c", "#dd104f")
            ),
            "B" to mutableMapOf(
                "env" to listOf("#181284"),
                "clo" to listOf("#491f49", "#ff9809", "#491f49"),
                "head" to listOf("#f6ba97"),
                "mouth" to listOf("#ff9809", "#000"),
                "eyes" to listOf("#c4ffe4", "#000", "#c4ffe4", "#000", "#000", "#000"),
                "top" to listOf("none", "none", "#d6f740", "#516303")
            ),
            "C" to mutableMapOf(
                "env" to listOf("#bcf700"),
                "clo" to listOf("#ff14e4", "#000", "#14fffd"),
                "head" to listOf("#7b401e"),
                "mouth" to listOf("#666", "#000"),
                "eyes" to listOf("#00b5b4", "#000", "#00b5b4", "#000", "#000", "#000"),
                "top" to listOf("#14fffd", "#14fffd", "#14fffd", "#0d3a62")
            )
        ),
        ...
        "08" to mutableMapOf(
            "A" to mutableMapOf(
                "env" to listOf("#0df"),
                "clo" to listOf("#571e57", "#ff0"),
                "head" to listOf("#f2c280"),
                "eyes" to listOf("#795548", "#000"),
                "mouth" to listOf("#ff0000"),
                "top" to listOf("#de3b00", "none")
            ),
            "B" to mutableMapOf(
                "env" to listOf("#B400C2"),
                "clo" to listOf("#0D204A", "#00ffdf"),
                "head" to listOf("#ca8628"),
                "eyes" to listOf("#cbbdaf", "#000"),
                "mouth" to listOf("#1a1a1a"),
                "top" to listOf("#000", "#000")
            ),
            "C" to mutableMapOf(
                "env" to listOf("#ffe926"),
                "clo" to listOf("#00d6af", "#000"),
                "head" to listOf("#8c5100"),
                "eyes" to listOf("none", "#000"),
                "mouth" to listOf("#7d0000"),
                "top" to listOf("#f7f7f7", "none")
            )
        ),
        ...
        "09" to mutableMapOf(
            "A" to mutableMapOf(
                "env" to listOf("#4aff0c"),
                "clo" to listOf("#101010", "#fff", "#fff"),
                "head" to listOf("#dbbc7f"),
                "mouth" to listOf("#000"),
                "eyes" to listOf("#000", "none", "none"),
                "top" to listOf("#531148", "#531148", "#531148", "none")
            ),
            "B" to mutableMapOf(
                "env" to listOf("#FFC107"),
                "clo" to listOf("#033c58", "#fff", "#fff"),
                "head" to listOf("#dbc97f"),
                "mouth" to listOf("#000"),
                "eyes" to listOf("none", "#fff", "#000"),
                "top" to listOf("#FFEB3B", "#FFEB3B", "none", "#FFEB3B")
                // "top": ["#085258", "#085258", "none", "#085258"]
            ),
            "C" to mutableMapOf(
                "env" to listOf("#FF9800"),
                "clo" to listOf("#b40000", "#fff", "#fff"),
                "head" to listOf("#E2AF6B"),
                "mouth" to listOf("#000"),
                "eyes" to listOf("none", "#fff", "#000"),
                "top" to listOf("#ec0000", "#ec0000", "none", "none")
            )
        ),
        ...
        "10" to mutableMapOf(
            "A" to mutableMapOf(
                "env" to listOf("#104c8c"),
                "clo" to listOf("#354B65", "#3D8EBB", "#89D0DA", "#00FFFD"),
                "head" to listOf("#cc9a5c"),
                "mouth" to listOf("#222", "#fff"),
                "eyes" to listOf("#000", "#000"),
                "top" to listOf("#fff", "#fff", "none")
            ),
            "B" to mutableMapOf(
                "env" to listOf("#0DC15C"),
                "clo" to listOf("#212121", "#fff", "#212121", "#fff"),
                "head" to listOf("#dca45f"),
                "mouth" to listOf("#111", "#633b1d"),
                "eyes" to listOf("#000", "#000"),
                "top" to listOf("none", "#792B74", "#792B74")
            ),
            "C" to mutableMapOf(
                "env" to listOf("#ffe500"),
                "clo" to listOf("#1e5e80", "#fff", "#1e5e80", "#fff"),
                "head" to listOf("#e8bc86"),
                "mouth" to listOf("#111", "none"),
                "eyes" to listOf("#000", "#000"),
                // "eyes": ["#636363", "#ADADAD", "#301e19"],
                "top" to listOf("none", "none", "#633b1d")
            )
        ),
        ...
        "11" to mutableMapOf(
            "A" to mutableMapOf(
                "env" to listOf("#4a3f73"),
                "clo" to listOf("#e6e9ee", "#f1543f", "#ff7058", "#fff", "#fff"),
                "head" to listOf("#b27e5b"),
                "mouth" to listOf("#191919", "#191919"),
                "eyes" to listOf("#000", "#000", "#57FFFD"),
                "top" to listOf("#ffc", "#ffc", "#ffc")
            ),
            "B" to mutableMapOf(
                "env" to listOf("#00a08d"),
                "clo" to listOf("#FFBA32", "#484848", "#4e4e4e", "#fff", "#fff"),
                "head" to listOf("#ab5f2c"),
                "mouth" to listOf("#191919", "#191919"),
                //  "#61b7ff",
                "eyes" to listOf("#000", "#ff23fa63", "#000"),
                "top" to listOf("#ff90f4", "#ff90f4", "#ff90f4")
            ),
            "C" to mutableMapOf(
                "env" to listOf("#22535d"),
                "clo" to listOf("#000", "#ff2500", "#ff2500", "#fff", "#fff"),
                "head" to listOf("#a76c44"),
                "mouth" to listOf("#191919", "#191919"),
                "eyes" to listOf("#000", "none", "#000"),
                "top" to listOf("none", "#00efff", "none")
            )
        ),
        ...
        "12" to mutableMapOf(
            "A" to mutableMapOf(
                "env" to listOf("#2668DC"),
                "clo" to listOf("#2385c6", "#b8d0e0", "#b8d0e0"),
                "head" to listOf("#ad8a60"),
                "mouth" to listOf("#000", "#4d4d4d"),
                "eyes" to listOf("#7fb5a2", "#d1eddf", "#301e19"),
                // "top": ["#ff6"]
                // "top": ["#ff0"]
                "top" to listOf("#fff510", "#fff510")
            ),
            "B" to mutableMapOf(
                "env" to listOf("#643869"),
                "clo" to listOf("#D67D1B", "#b8d0e0", "#b8d0e0"),
                "head" to listOf("#CC985A", "none0000"),
                "mouth" to listOf("#000", "#ececec"),
                "eyes" to listOf("#1f2644", "#9b97ce", "#301e19"),
                // "top": ["#c700c7"]
                // "top": ["#c54560"]
                // "top": ["#ff393e", "none"]
                "top" to listOf("#00eaff", "none")
            ),
            "C" to mutableMapOf(
                "env" to listOf("#F599FF"),
                "clo" to listOf("#2823C6", "#b8d0e0", "#b8d0e0"),
                "head" to listOf("#C7873A"),
                "mouth" to listOf("#000", "#4d4d4d"),
                "eyes" to listOf("#581b1b", "#FF8B8B", "#000"),
                // "top": ["#5F1F1F"]
                // "top": ["#fff00"]
                "top" to listOf("none", "#9c0092")
            )
        ),
        ...
        "13" to mutableMapOf(
            "A" to mutableMapOf(
                "env" to listOf("#d10084"),
                "clo" to listOf("#efedee", "#00a1e0", "#00a1e0", "#efedee", "#ffce1c"),
                "head" to listOf("#b35f49"),
                "mouth" to listOf("#3a484a", "#000"),
                "eyes" to listOf("#000"),
                "top" to listOf("#000", "none", "#000", "none")
            ),
            "B" to mutableMapOf(
                "env" to listOf("#E6C117"),
                "clo" to listOf("#efedee", "#ec0033", "#ec0033", "#efedee", "#f2ff05"),
                "head" to listOf("#ffc016"),
                "mouth" to listOf("#4a3737", "#000"),
                "eyes" to listOf("#000"),
                "top" to listOf("#ffe900", "#ffe900", "none", "#ffe900")
            ),
            "C" to mutableMapOf(
                "env" to listOf("#1d8c00"),
                // "env": ["#C6FE3C"],
                // "env": ["#cace10"],
                "clo" to listOf("#e000cb", "#fff", "#fff", "#e000cb", "#ffce1c"),
                "head" to listOf("#b96438"),
                "mouth" to listOf("#000", "#000"),
                "eyes" to listOf("#000"),
                // "top": ["#eff4b0"]
                // "top": ["#e0e0e0"]
                // "top": ["#d7ffff"] // ice
                // "top": ["#f0ffff"] // ice
                "top" to listOf("#53ffff", "#53ffff", "none", "none")
            )
        ),
        ...
        "14" to mutableMapOf(
            "A" to mutableMapOf(
                "env" to listOf("#fc0065"),
                "clo" to listOf("#708913", "#fdea14", "#708913", "#fdea14", "#708913"),
                "head" to listOf("#DEA561"),
                "mouth" to listOf("#444", "#000"),
                "eyes" to listOf("#000"),
                "top" to listOf(
                    "#32393f",
                    "#32393f",
            ...
            "B" to mutableMapOf(
                "env" to listOf("#81f72e"),
                "clo" to listOf("#ff0000", "#ffc107", "#ff0000", "#ffc107", "#ff0000"),
                "head" to listOf("#ef9831"),
                // "mouth": ["#a2a2a2", "#000"],
                "mouth" to listOf("#6b0000", "#000"),
                "eyes" to listOf("#000"),
                "top" to listOf(
                    "#FFFAAD",
                    "#FFFAAD",
            ...
            "C" to mutableMapOf(
                "env" to listOf("#00D872"),
                "clo" to listOf("#590D00", "#FD1336", "#590D00", "#FD1336", "#590D00"),
                "head" to listOf("#c36c00"),
                "mouth" to listOf("#56442b", "#000"),
                "eyes" to listOf("#000"),
                "top" to listOf(
                    "#004E4C",
                    "#004E4C",
        ...
        "15" to mutableMapOf(
            "A" to mutableMapOf(
                // "env": ["#ccc"],
                "env" to listOf("#111"),
                "clo" to listOf("#000", "#00FFFF"),
                "head" to listOf("#755227"),
                "mouth" to listOf("#fff", "#000"),
                "eyes" to listOf("black", "#66008a", "aqua"),
                // "top": ["#004a57"] // dark blue
                "top" to listOf("#fff", "#fff", "#fff", "#fff", "#fff")
            ),
            "B" to mutableMapOf(
                "env" to listOf("#00D0D4"),
                // "env": ["#30afab"],
                // "env": ["#E8E8E8"],
                "clo" to listOf("#000", "#fff"),
                "head" to listOf("#755227"),
                ...
                "eyes" to listOf("black", "#1df7ffa3", "#fcff2c"),
                // "top": ["#fffbaf"] // bright yellow
                "top" to listOf("#fff539", "none", "#fff539", "none", "#fff539")
            ),
            "C" to mutableMapOf(
                "env" to listOf("#DC75FF"),
                "clo" to listOf("#000", "#FFBDEC"),
                "head" to listOf("#997549"),
                "mouth" to listOf("#fff", "#000"),
                "eyes" to listOf("black", "black", "aqua"),
                // "top": ["#474747"]
                // "top": ["#ff64e6"]
                "top" to listOf("#00fffd", "none", "none", "none", "none")
            )
        )
    )

    val sP: MutableMap<String, MutableMap<String, String>> = mutableMapOf(
        "00" to mutableMapOf<String, String>(),
        "01" to mutableMapOf<String, String>(),
        "02" to mutableMapOf<String, String>(),
        "03" to mutableMapOf<String, String>(),
        "04" to mutableMapOf<String, String>(),
        "05" to mutableMapOf<String, String>(),
        "06" to mutableMapOf<String, String>(),
        "07" to mutableMapOf<String, String>(),
        "08" to mutableMapOf<String, String>(),
        "09" to mutableMapOf<String, String>(),
        "10" to mutableMapOf<String, String>(),
        "11" to mutableMapOf<String, String>(),
        "12" to mutableMapOf<String, String>(),
        "13" to mutableMapOf<String, String>(),
        "14" to mutableMapOf<String, String>(),
        "15" to mutableMapOf<String, String>()
    )
    const val svgStart = "<svg xmlns=\"http://www.w3.org/2000/svg\" viewBox=\"0 0 231 231\">"
    const val svgEnd = "</svg>"
    ...
    const val str = "stroke-linecap:round;stroke-linejoin:round;stroke-width:"
    ...
    init {
        ...
        sP["07"]?.set(
            "top",
            "<path d=\"m30.622 70.381c2.0971-3.9374 4.6649-7.9604 7.6822-12.037 3.0172-4.0765 6.0987-7.6929 9.2229-10.817l22.897 22.897c-4.4402 4.4403-8.2278 9.5439-11.213 15.14z\" style=\"fill:#999;\"/><path d=\"m160.58 70.423 22.907-22.897c3.1242 3.1242 6.2056 6.7406 9.2229 10.817 3.0065 4.0765 5.5744 8.0994 7.6715 12.037 3.2025 13.403 3.8346 25.22 2.9106 42.253l11.172-0.23161c1.4706-11.886 3.8989-29.213 2.1636-42.021 10.416 12.631 11.373 23.624 13.077 39.726 30.174-0.76004 59.808-4.5121 77.845-10.128-10.76-38.608-41.475-55.66-80.38-56.104-38.182-0.45134-74.543 22.405-82.065 57.432z\" style=\"fill:#999;\"/>"
        )
        ...
        sP["09"]?.set(
            "eyes",
            "<path d=\"m83.527 103.98v10h10v-10h-10zm53.945 0v10h10v-10h-10z\" style=\"fill:#1a1a1a;\"/>" +
                    "<path d=\"m56.621 94.906v11.688h5.3418v6.4922h5.3418v6.1055h5.3223v6.2324h26.846v-6.2324h5.3047v-6.1055h5.1445v-6.0039h11.154v6.0039h5.1446v6.1055h5.3066v6.2324h26.846v-6.2324h5.3203v-6.1055h5.3438v-6.4922h5.3418v-11.688z\" style=\"fill:#1a1a1a;\"/>" +
    ...
    fun getAvatarSvgBytes(input: AvatarData, transparentBackground: Boolean = false): ByteArray =
        getAvatarSvgString(input, transparentBackground)
            .encodeToByteArray()

    fun getAvatarSvgString(input: AvatarData, transparentBackground: Boolean = false): String {
        val p: MutableMap<String, String> = mutableMapOf()

        p["env"] = input.background.value.toString()
        p["clo"] = input.clothes.value.toString()
        p["head"] = input.head.value.toString()
        p["mouth"] = input.mouth.value.toString()
        p["eyes"] = input.eyes.value.toString()
        p["top"] = input.top.value.toString()

        // Adjust each part based on its numeric value
//        for (part in p.keys) {
//            var nr = p[part]!!
//            if (nr.toInt() > 31) {
//                nr = (nr.toInt() - 32).toString()
//                if (nr.length == 1) nr = "0" + nr
//                p[part] = nr + "C"
//            } else if (nr.toInt() > 15) {
//                nr = (nr.toInt() - 16).toString()
//                if (nr.length == 1) nr = "0" + nr
//                p[part] = nr + "B"
//            } else {
//                if (nr.length == 1) {
...
//        }
        ...
        for ((part, value) in p) {
            val nr = value.toInt()
            val (newNr, suffix) = when {
                nr > 31 -> nr - 32 to "C"
                nr > 15 -> nr - 16 to "B"
                else -> nr to "A"
            }
            val paddedNr = if (newNr < 10) "0$newNr" else "$newNr" // Zero-padding manually
            p[part] = "$paddedNr$suffix"
        }

        val final = mutableMapOf<String, String>()

        // Apply getFinal to each part using the previously computed values in p.
        for (part in p.keys) {
            val value = p[part]!!
            val partV = value.substring(0, 2)
            val themeValue = value.substring(2, 3)
            final[part] = getFinalSvgPart(part, partV, themeValue)
        }

        if (transparentBackground) final["env"] = ""

        // Concatenate all parts of the SVG to form the final output
        val svg = svgStart +
                final["env"].orEmpty() +
                final["head"].orEmpty() +
                final["clo"].orEmpty() +
    ...
    // Local function to generate final SVG string for each part.
    private fun getFinalSvgPart(part: String, partV: String, theme: String): String {
        // Retrieve the colors list from themes using safe calls
        val colors: List<String>? = themes[partV]?.get(theme)?.get(part)
        // Retrieve the SVG part string from sP using safe calls
        val svgString: String? = sP[partV]?.get(part)

        val regex = "(#.*?;)"
        var resultFinal = svgString ?: ""

        // Find all matches using the regex
        val result = regex.toRegex().findAll(svgString ?: "")
            .map { it.groupValues[1] }
            .toList()

        // Replace each match with the corresponding color value from colors
        for (i in result.indices) {
            resultFinal =
                resultFinal.replaceFirst(result[i], (colors?.getOrNull(i) ?: "") + ";")
        }
    ...
    fun sha256Hash(string: String): String {
        ...
        val sha256Digest = SHA256().digest(bytes)
        val sha256Hash =
            sha256Digest.joinToString("") { it.toUByte().toString(16).padStart(2, '0') }

        val sha256Numbers = sha256Hash.filter { it.isDigit() }
        return sha256Numbers.take(12)
    }
}
